...
SpellDisplay = (function () {
  var canvas, ctx, wizard, scale, spell_inks, current_spell;
  ...
  function fetch_spell_image(spell) {
    ...
    if (spell.stats_canvas === undefined) {
      ...
      if (spell.id <= 33) {
        stats_ctx.drawImage(Storage.border(256 * scale, 192 * scale, RGB.b_green, RGB.black, scale), 0, 0);
        stats_ctx.drawImage(Storage.text(Storage.in_game_message(79) + ((World.cast_chance(wizard, spell) + 1) * 10) + '%', RGB.b_cyan, scale), 32 * scale, 144 * scale);
        stats_ctx.drawImage(Info.stats(Storage.create_object(spell.id)), 32 * scale, 16 * scale);
      } else {
        stats_ctx.drawImage(Storage.border(256 * scale, 192 * scale, RGB.b_blue, RGB.b_cyan, scale), 0, 0);
        stats_ctx.drawImage(Storage.text(spell.name, RGB.b_yellow, scale), 64 * scale, 40 * scale);
        if (spell.chaos_law_value > 0) {
          stats_ctx.drawImage(Storage.text(Storage.in_game_message(71) + ' ' + spell.chaos_law_value + ')', RGB.b_cyan, scale), 64 * scale, 56 * scale);
        } else if (spell.chaos_law_value < 0) {
          stats_ctx.drawImage(Storage.text(Storage.in_game_message(70) + ' ' + Math.abs(spell.chaos_law_value) + ')', RGB.b_purple, scale), 64 * scale, 56 * scale);
        }
        cast_range = Math.floor(spell.doubled_cast_range / 2);
        stats_ctx.drawImage(Canvas.tile_horizontal([Storage.text(Storage.in_game_message(79), RGB.b_green, scale), Storage.text(((World.cast_chance(wizard, spell) + 1) * 10) + '%', RGB.b_yellow, scale)]), 64 * scale, 80 * scale);
        stats_ctx.drawImage(Canvas.tile_horizontal([Storage.text(Storage.in_game_message(74), RGB.b_green, scale), Storage.text((cast_range > 9) ? String(20) : String(cast_range), RGB.b_yellow, scale)]), 64 * scale, 112 * scale);
      }
    }
  ...
  function show_spell_info(spell_number) {
    ...
    if (spell_number !== undefined) {
      ...
      if (spell) {
        ctx.drawImage(fetch_spell_image(spell), 0, 192);
      }
    }
  }
